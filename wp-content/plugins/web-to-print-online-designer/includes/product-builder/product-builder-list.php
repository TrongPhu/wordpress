...
class NB_Productbuilder_List_Table extends WP_List_Table {

    public function __construct() {
        parent::__construct(array(
            'singular'  => __('Product builder', 'web-to-print-online-designer'),
            'plural'    => __('Product builder', 'web-to-print-online-designer'),
            'ajax'      => false
        ));
    }
    ...
    public static function record_count() {
        global $wpdb;
        $sql = "SELECT COUNT(*) FROM {$wpdb->prefix}nb_product_builder";
        //filter
    ...
    public function process_bulk_action() {
        if ( 'delete' === $this->current_action() ) {
            ...
            wp_redirect( esc_url_raw( add_query_arg( array( 'paged' => $this->get_pagenum() ), admin_url( 'admin.php?page=nb_product_builder' ) ) ) );
            exit;
        }
        if ( 'copy' === $this->current_action() ) {
            ...
            wp_redirect( esc_url_raw( admin_url( 'admin.php?page=nb_product_builder' ) ) );
            exit;
        }
    ...
    public function delete_option( $id ){
        global $wpdb;
        $sql = "DELETE FROM {$wpdb->prefix}nb_product_builder";
    ...
    public function unpublish_option( $id ){
        global $wpdb;
        $result = $wpdb->update($wpdb->prefix . 'nb_product_builder', array(
            'published' => 0
        ), array( 'id' => esc_sql($id)));
        if( $result ) $this->clear_transients();
    }
    ...
    public function copy_options( $id ){
        ...
        if( count( $result[0] ) ){
            ...
            $in_res = $wpdb->insert("{$wpdb->prefix}nb_product_builder", $arr);
            if( $in_res ){
